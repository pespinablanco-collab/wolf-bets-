...
const ADMIN_DRAFT_KEY = "wolfbets_admin_match_draft_v1";
...
function renderMatches() {
  ...
  root.innerHTML = matches.map(match => {
    ...
    const adminDots = admin ? `
      <div class="match-admin-menu-wrap">
        <button class="match-admin-dots" data-match-menu="${match.id}" aria-label="Opciones del partido">⋮</button>
        <div class="match-admin-menu hidden" data-match-menu-panel="${match.id}">
          <button data-match-edit="${match.id}">✏️ Editar partido</button>
          <button data-match-live="${match.id}">⚽ Editar directo</button>
          <button data-match-markets="${match.id}">🎯 Gestionar selecciones</button>
          <button data-match-close="${match.id}">🔒 Cerrar apuestas</button>
          <button class="danger" data-match-delete="${match.id}">🗑️ Eliminar partido</button>
        </div>
      </div>` : "";
    ...
    if (!ms.length) {
      return `<article class="card match-card">${header}<div class="empty-state">No hay mercados abiertos.</div></article>`;
    }
      ...
      <div class="market-group">
        ...
        <div class="market-list">
          ${list.map(m => {
            ...
            return `<div class="market-row ${selected ? "selected" : ""}">
              <button class="market-main ${selected ? "selected" : ""}" data-market-id="${m.id}" title="Añadir a combinada">
                <span class="market-name">${esc(m.name)}${s ? `<span class="super-badge">⚡ ${esc(s.label || "SUPERCUOTA")}</span>` : ""}</span>
                <span class="odd-wrap">${s ? `<del class="old-odd">${Number(s.original_odd).toFixed(2)}</del><span class="odd super-odd">${Number(m.odd).toFixed(2)}</span>` : `<span class="odd">${Number(m.odd).toFixed(2)}</span>`}</span>
              </button>
    ...
    return `<article class="card match-card">${header}<div class="market-grid">${marketHtml}</div></article>`;
  }).join("");

  root.querySelectorAll("[data-market-id]").forEach(btn => btn.addEventListener("click", () => toggleCombo(btn.dataset.marketId)));
  root.querySelectorAll("[data-simple-id]").forEach(btn => btn.addEventListener("click", () => {
    const market = markets.find(m => String(m.id) === String(btn.dataset.simpleId));
    if (market) placeSimpleBet(market);
  }));

  root.querySelectorAll("[data-match-menu]").forEach(btn => btn.addEventListener("click", e => {
    e.stopPropagation();
    const id = btn.dataset.matchMenu;
    root.querySelectorAll("[data-match-menu-panel]").forEach(p => {
      if (p.dataset.matchMenuPanel !== id) p.classList.add("hidden");
    });
    root.querySelector(`[data-match-menu-panel="${id}"]`)?.classList.toggle("hidden");
  }));
  root.querySelectorAll("[data-match-edit]").forEach(btn => btn.addEventListener("click", () => adminOpenMatchEditor(Number(btn.dataset.matchEdit))));
  root.querySelectorAll("[data-match-live]").forEach(btn => btn.addEventListener("click", () => openLiveEditor(Number(btn.dataset.matchLive))));
  root.querySelectorAll("[data-match-markets]").forEach(btn => btn.addEventListener("click", () => adminOpenMarkets(Number(btn.dataset.matchMarkets))));
  root.querySelectorAll("[data-match-close]").forEach(btn => btn.addEventListener("click", async () => {
    const id = Number(btn.dataset.matchClose);
    if (!confirm("¿Cerrar todas las apuestas de este partido? No se borrará el historial.")) return;
    await adminAction(async () => {
      const { error } = await sb.rpc("admin_close_match_markets", { p_match_id:id, p_status:"closed" });
      if (error) throw error;
    }, "Apuestas cerradas.");
  }));
  root.querySelectorAll("[data-match-delete]").forEach(btn => btn.addEventListener("click", async () => {
    const id = Number(btn.dataset.matchDelete);
    const m = matches.find(x => Number(x.id) === id);
    if (!m) return;
    if (!confirm(`¿Eliminar ${m.home} vs ${m.away}?\n\nEsta acción solo será posible si el partido no tiene apuestas ni combinadas.`)) return;
    await adminAction(async () => {
...
function adminOpenMatchEditor(id) {
  const m = adminMatches.find(x => Number(x.id) === Number(id)) || matches.find(x => Number(x.id) === Number(id));
  if (!m) return;
...
function openLiveEditor(id) {
  const m = adminMatches.find(x => Number(x.id) === Number(id)) || matches.find(x => Number(x.id) === Number(id));
  if (!m) return;
  const modal = $("liveEditModal");
  if (!modal) return;
  $("liveEditMatchId").value = id;
  $("liveEditTitle").textContent = `${m.home} vs ${m.away}`;
  $("liveEditHome").value = Number(m.score_home ?? 0);
  $("liveEditAway").value = Number(m.score_away ?? 0);
  $("liveEditMinute").value = m.live_minute ?? "";
  $("liveEditIsLive").checked = Boolean(m.is_live);
  modal.classList.remove("hidden");
}

async function saveLiveEditor(finish=false) {
  const id = Number($("liveEditMatchId").value);
  const home = Number($("liveEditHome").value);
  const away = Number($("liveEditAway").value);
  const raw = $("liveEditMinute").value.trim();
  ...
  if (!Number.isInteger(home) || home < 0 || !Number.isInteger(away) || away < 0 || (minute !== null && (!Number.isInteger(minute) || minute < 0))) {
    msg("Marcador/minuto no válidos.", "error"); return;
  }
  if (finish && !confirm(`¿Finalizar el partido con ${home}-${away}?`)) return;
  await adminAction(async () => {
    const rpc = finish ? "admin_finish_match" : "admin_update_live_result";
    const params = finish
      ? { p_match_id:id, p_score_home:home, p_score_away:away }
      : { p_match_id:id, p_score_home:home, p_score_away:away, p_live_minute:minute, p_is_live:Boolean($("liveEditIsLive").checked), p_status:null };
    const { error } = await sb.rpc(rpc, params);
    if (error) throw error;
  }, finish ? "Partido finalizado." : "Resultado en directo actualizado.");
  $("liveEditModal")?.classList.add("hidden");
}
...
function readAdminDraft() {
  try { return JSON.parse(localStorage.getItem(ADMIN_DRAFT_KEY) || "null"); } catch (_) { return null; }
}
function saveAdminDraft() {
  if ($( "adminMatchId")?.value) return;
  const draft = {
    home:$('adminHome').value, away:$('adminAway').value, date:$('adminDate').value,
    sport:$('adminSport').value, competition:$('adminCompetition').value,
    oddHome:$('adminOddHome').value, oddDraw:$('adminOddDraw').value, oddAway:$('adminOddAway').value,
    status:$('adminStatus').value, result:$('adminResult').value, scoreHome:$('adminScoreHome').value, scoreAway:$('adminScoreAway').value,
    homeLogo:$('adminHomeLogo').value, awayLogo:$('adminAwayLogo').value, homeShirt:$('adminHomeShirt').value, awayShirt:$('adminAwayShirt').value,
    matchImage:$('adminMatchImage').value, banner:$('adminBanner').value, featured:$('adminFeatured').checked, sortOrder:$('adminSortOrder').value,
    markets: adminDraftMarkets
  };
  localStorage.setItem(ADMIN_DRAFT_KEY, JSON.stringify(draft));
}
function clearAdminDraft() { localStorage.removeItem(ADMIN_DRAFT_KEY); adminDraftMarkets=[]; }
function restoreAdminDraft() {
  const d=readAdminDraft();
  if (!d) return false;
  $('adminMatchId').value=""; $('adminEditorTitle').textContent="Nuevo partido (borrador)";
  $('adminHome').value=d.home||""; $('adminAway').value=d.away||""; $('adminDate').value=d.date||localDateTimeValue(new Date(Date.now()+3600000));
  $('adminSport').value=d.sport||"Fútbol"; $('adminCompetition').value=d.competition||"";
  $('adminOddHome').value=d.oddHome||"2.00"; $('adminOddDraw').value=d.oddDraw||"3.00"; $('adminOddAway').value=d.oddAway||"2.00";
  $('adminStatus').value=d.status||"open"; $('adminResult').value=d.result||""; $('adminScoreHome').value=d.scoreHome||""; $('adminScoreAway').value=d.scoreAway||"";
  $('adminHomeLogo').value=d.homeLogo||""; $('adminAwayLogo').value=d.awayLogo||""; $('adminHomeShirt').value=d.homeShirt||""; $('adminAwayShirt').value=d.awayShirt||"";
  $('adminMatchImage').value=d.matchImage||""; $('adminBanner').value=d.banner||""; $('adminFeatured').checked=Boolean(d.featured); $('adminSortOrder').value=d.sortOrder||"0";
  adminDraftMarkets=Array.isArray(d.markets)?d.markets:[]; renderDraftMarkets(); return true;
}
function renderDraftMarkets() {
  const root=$("adminDraftMarkets"); if(!root) return;
  root.innerHTML=adminDraftMarkets.length ? adminDraftMarkets.map((m,i)=>`<div class="draft-market-row"><span><strong>${esc(m.name)}</strong><small>${esc(m.category)} · ${Number(m.odd).toFixed(2)}</small></span><button type="button" class="btn btn-danger admin-mini" data-draft-remove="${i}">Eliminar</button></div>`).join("") : `<div class="muted">No hay selecciones guardadas en el borrador.</div>`;
  root.querySelectorAll('[data-draft-remove]').forEach(b=>b.onclick=()=>{adminDraftMarkets.splice(Number(b.dataset.draftRemove),1);saveAdminDraft();renderDraftMarkets();});
}
function addDraftMarket() {
  const category=prompt("Categoría", "Resultado"); if(!category) return;
  const name=prompt("Nombre de la selección", "Gana local"); if(!name) return;
  const odd=Number(prompt("Cuota", "2.00")); if(!Number.isFinite(odd)||odd<=1){adminMsg("La cuota debe ser mayor que 1.","error");return;}
  adminDraftMarkets.push({category:category.trim(),name:name.trim(),odd,sort_order:adminDraftMarkets.length}); saveAdminDraft(); renderDraftMarkets();
}

function adminResetForm(clearDraft = false) {
  $("adminMatchId").value = "";
  ...
  adminDraftMarkets = [];
  if (!clearDraft) restoreAdminDraft();
  renderDraftMarkets();
}
...
async function adminSaveMatch(e) {
  ...
  await adminAction(async () => {
    ...
    if (!id && matchId && adminDraftMarkets.length) {
      for (const dm of adminDraftMarkets) {
        const { error: marketError } = await sb.rpc("admin_create_market", { p_match_id:matchId, p_category:dm.category, p_name:dm.name, p_odd:Number(dm.odd), p_is_open:true, p_sort_order:Number(dm.sort_order||0) });
...
function initAdminPanel() {
  ...
  $("adminNewMatchBtn").onclick = () => { adminResetForm(false); $("adminEditor").scrollIntoView({behavior:"smooth", block:"start"}); };
  $("adminCancelEditBtn").onclick = () => { saveAdminDraft(); adminMsg("Borrador guardado. No se han borrado imágenes ni selecciones.", "ok"); };
  ...
  $("adminAddDraftMarketBtn").onclick = addDraftMarket;
  document.querySelectorAll("#adminMatchForm input, #adminMatchForm select").forEach(el => el.addEventListener("input", saveAdminDraft));
  document.querySelectorAll("#adminMatchForm input, #adminMatchForm select").forEach(el => el.addEventListener("change", saveAdminDraft));
  adminResetForm(false);
...
$("liveEditCloseBtn")?.addEventListener("click", () => $("liveEditModal")?.classList.add("hidden"));
$("liveEditSaveBtn")?.addEventListener("click", () => saveLiveEditor(false));
$("liveEditFinishBtn")?.addEventListener("click", () => saveLiveEditor(true));
$("liveEditModal")?.addEventListener("click", e => { if (e.target.id === "liveEditModal") e.currentTarget.classList.add("hidden"); });
document.addEventListener("click", e => { if (!e.target.closest(".match-admin-menu-wrap")) document.querySelectorAll("[data-match-menu-panel]").forEach(p=>p.classList.add("hidden")); });
